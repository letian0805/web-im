    ...
    conn.listen({
        ...
        onPresence : function(message) {
            if ( message.type == 'subscribe' ) {
                newScribe.push(message);
                newScribe.handling || handlePresence(newScribe.shift());
            } else {
...
var handlePresence = function(e) {
    ...
    if (e.type == 'subscribe') {
        ...
        $('#confirm-block-footer-confirmButton').click(function() {
            ...
            if (newScribe.length > 0 ) {
                handlePresence(newScribe.shift());
            } else {
                newScribe.handling = false;
            }
            return false;
        });
...
var showEmotionDialog = function() {
    ...
    for ( var key in sjson.map) {
        var emotions = $('<img>').attr({
            "id" : key,
            "src" : sjson.path + sjson.map[key],
...
var sendText = function() {
    ...
    // 群组消息和个人消息的判断分支
    var isGroup = curChatUserId.indexOf(groupFlagMark) >= 0;
    var to = isGroup ? curRoomId : curChatUserId;

    if (to == null) {
        return;
    }
    
    var message = new Message('txt', conn.getUniqueId());
    ...
    isGroup && message.setGroup('groupchat');
    conn.send(message.body);

    //当前登录人发送的信息在聊天窗口中原样显示
    appendMsg(curUserId, to, message.get());
    turnoffFaces_box();
    msgInput.value = "";
    msgInput.focus();
    setTimeout(function() {
...
var realFile = $("#fileInput");
realFile.on('change', function () {
    var t = $(this),
        idx = t.val().lastIndexOf('.');
        postFix = idx > 0 ? t.val().slice(idx + 1) : '';

    if ( postFix in pictype ) {
       sendPic();
    } else if ( postFix in audiotype ) {
        sendAudio();
    } else if ( postFix in filetype ) {
        sendFile();
    } else {
        alert('当前不支持发送' + postFix + '格式的文件');
    }
});
...
var sendPic = function( file ) {
    ...
    var message = new Message('img', conn.getUniqueId());

    message.set({
        file: file || Easemob.im.Utils.getFileUrl(realFile.attr('id')),
        to: to,
...
var sendAudio = function( file ) {
    ...
    var message = new Message('audio', conn.getUniqueId());
...
var sendFile = function( file ) {
    ...
    var message = new Message('file', conn.getUniqueId());
    ...
    message.set({
        ...
        uploadComplete: function ( data, id ) {
            $('#' + id).find('a').attr('href', data.uri + '/' + data.entities[0].uuid);
        },
        success: function ( id ) {
            $('#' + id + '_loading').remove();
            $('#' + id + '_failed').remove();
...
var handleTextMessage = function(message) {
    ...
    if (mestype == 'groupchat') {
        appendMsg(message.from, message.to, message.get(true), mestype);
    } else {
        appendMsg(from, from, message.get(true));
    }
};
...
var handlePictureMessage = function(message) {
    ...
    appendMsg(from, contactDivId, msg.get(true));
};
...
var handleFileMessage = function(message) {
    ...
    msg.set({file: {url: message.url, filename: filename}});
    appendMsg(from, contactDivId, msg.get(true));
};
//收到视频消息
 ...
 * 各种消息类型
 */
var Message = Easemob.im.EmMessage;
//文本消息
...
Message.txt.prototype.get = function ( isReceive ) {
    ...
    return [
        !isReceive ? "<div id='" + this.id + "' class='right'>" : "<div class='left'>",
            this.id ? "<span id='" + this.id + "_failed' class='hide'>发送失败.点击重新发送</span>" : "",
            this.id ? "<span id='" + this.id + "_loading'>发送中</span>" : "",
            "<p>" + Easemob.im.Utils.parseLink(Easemob.im.Utils.parseEmotions(encode(this.value))) + "</p>",
        "</div>"
    ].join('');
};
...
Message.img.prototype.get = function ( isReceive ) {
    return [
        !isReceive ? "<div id='" + this.id + "' class='right'>" : "<div class='left'>",
            this.id ? "<span id='" + this.id + "_failed' class='hide'>发送失败</span>" : "",
            this.id ? "<span id='" + this.id + "_loading'>发送中</span>" : "",
            "<a class='easemobWidget-noline' href='" + this.value.url + "' target='_blank'><img src='" + this.value.url + "'/></a>",
        "</div>"
    ].join('');
}
...
Message.audio.prototype.get = function ( isReceive ) {
    var audio = '';
    if ( 'Audio' in window ) {
        audio = "<audio controls src='" + this.value.url + "'/>";
    } else {
        audio = "当前浏览器不支持播放，音频地址:<a href='" + this.value.url + "'>" + this.value.url + "</a>";
    }
...
Message.file.prototype.get = function ( isReceive ) {
    return [
        !isReceive ? "<div id='" + this.id + "' class='right'>" : "<div class='left'>",
            this.id ? "<span id='" + this.id + "_failed'>发送失败</span>" : "",
            this.id ? "<span id='" + this.id + "_loading'>发送中</span>" : "",
            "<a href='" + this.value.url + "' target='_blank'>" + this.value.filename + "</a>",
        "</div>"
    ].join('');
}
